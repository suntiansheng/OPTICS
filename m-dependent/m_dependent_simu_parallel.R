...
# -----------------------------------------------------------------------------
# Parallel simulation for m-dependent mean-change model
...
run_setting_parallel_mdep <- function(n, p, cps, a_grid, M, simu_time = 100, cl) {
  model <- 1:ceiling(log(n))
  true_model <- length(cps)
  res <- matrix(NA_real_, nrow = length(a_grid), ncol = 17)
  ...
  colnames(res) <- c(
    "a", "M_OPTICS(BS)_cov", "M_OPTICS(BS)_len", "M_OPTICS(SN)_cov", "M_OPTICS(SN)_len",
...
res_mdep <- run_setting_parallel_mdep(n = n, p = p, cps = cps, a_grid = a_grid, M = M, simu_time = simu_time, cl = cl)
write.csv(res_mdep, file = "./results/m_dependent.csv", row.names = FALSE)
