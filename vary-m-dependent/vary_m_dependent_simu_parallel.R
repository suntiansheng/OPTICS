...
# -----------------------------------------------------------------------------
# Parallel simulation for varying m-dependent level (fixed amplitude)
...
run_one_rep_mdep <- function(n, p, cps, a, model, true_model, M) {
  ...
  c(
    ...
    M_OPTICS_SN_cov = as.numeric(true_model %in% (m_cv_sn$A - 1)),
...
run_setting_parallel_mdep <- function(n, p, cps, a, M_grid, simu_time = 100, cl) {
  model <- 1:ceiling(log(n))
  true_model <- length(cps)
  res <- matrix(NA_real_, nrow = length(M_grid), ncol = 17)

  for (iter in seq_along(M_grid)) {
    M <- M_grid[iter]
    message(sprintf("[%s] M=%d (%d/%d)", Sys.time(), M, iter, length(M_grid)))

    rep_list <- parLapply(
      cl = cl,
      X = seq_len(simu_time),
      fun = function(i, n, p, cps, a, model, true_model, M) {
        run_one_rep_mdep(n = n, p = p, cps = cps, a = a, model = model, true_model = true_model, M = M)
      },
      n = n,
      p = p,
      cps = cps,
    ...
    res[iter, ] <- c(M, colMeans(rep_mat))
    print(res[iter, ])
  }

  colnames(res) <- c(
    "M", "M_OPTICS(BS)_cov", "M_OPTICS(BS)_len", "M_OPTICS(SN)_cov", "M_OPTICS(SN)_len",
    "OPTICS(BS)_cov", "OPTICS(BS)_len", "OPTICS(SN)_cov", "OPTICS(SN)_len",
    "A_COPSS(BS)", "A_COPSS(SN)", "A_FDRseg", "A_SMUCE",
    "COPSS(BS)", "COPSS(SN)", "fdr_seg", "smuce"
  )
  res
}

# -----------------------------------------------------------------------------
# Experiment setting (fixed a, varying M)
# -----------------------------------------------------------------------------

set.seed(1234)
n <- 2000
p <- 1
cps <- c(400, 800, 1200, 1600)
a <- 0.75
M_grid <- c(1, 2, 3, 5, 8)
simu_time <- 100
...
clusterExport(
  cl,
  varlist = c("MCP_model_mdep", "run_one_rep_mdep"),
  envir = environment()
)
...
res_vary_m <- run_setting_parallel_mdep(
  n = n, p = p, cps = cps, a = a, M_grid = M_grid, simu_time = simu_time, cl = cl
)
write.csv(res_vary_m, file = "./results/vary_m_dependent.csv", row.names = FALSE)
